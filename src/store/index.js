  ...
  mutations: {
    ...
    removeFollowingUser(state, userid) {
      if (state.notFollowingUsers.length) {
        state.notFollowingUsers = state.notFollowingUsers.filter(
          (user) => user.id != userid
        );
      }
    },
  ...
  actions: {
    ...
    async getProfileUser({ state, commit }, username) {
      ...
      const user = await axios.get(
        `${ACTION_ROUTES.USER_DEFAULT_ROUTE}/username/${username}`
      );
      const following = user.data.id;
      if (follower != following) {
        const isFollowingProfileUser = await axios.get(
          `${ACTION_ROUTES.FOLLOW_DEFAULT_ROUTE}/isfollowing/${follower}/${following}`
        );
    ...
    async getUserTweets({ state, commit }) {
      ...
      const tweets = await axios.get(
        `${ACTION_ROUTES.TWEET_DEFAULT_ROUTE}/user/${userid}`
      );
    ...
    async getFeedTweets({ state, commit }) {
      ...
      const tweets = await axios.get(
        `${ACTION_ROUTES.TWEET_DEFAULT_ROUTE}/feed/${userid}`
      );
      commit("updateTweets", tweets);
    },
    ...
    async getNotFollowingUsers({ state, commit }, checkForThree) {
      if ( !checkForThree || state.notFollowingUsers.length < state.defaultLimit ) {
        ...
        const users = await axios.get(
          `${ACTION_ROUTES.USER_DEFAULT_ROUTE}/notfollowingusers/${userid}/${state.offset}/${state.limit}`
        );
